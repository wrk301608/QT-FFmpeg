...
GIFWidget *GIFWidget::getInstance()
{
    if (self.isNull()) {
        static QMutex mutex;
...
void GIFWidget::saveImage()
{
    ...
    gif.GifWriteFrame(gifWriter,
                      image.bits(),
                      rectGif.width(),
                      rectGif.height(),
                      fps);
    fpsCount++;
    labStatus->setText(QString("正在录制 第 %1 帧").arg(fpsCount));
}

//重写关闭
void GIFWidget::closeEvent(QCloseEvent * event)
{
    event->ignore();
    this->hide();
    if(mainWidget!=NULL)
...
void GIFWidget::setMainWidget(QWidget *mainWidget)
{
...
void GIFWidget::record()
{
    if(btnStart->text() == "开始")
    {
        ...
        bool ok = gif.GifBegin(gifWriter,
                               name,
                               width,
                               height,
                               fps);
    ...
    else
    {
        ...
        labStatus->setText(QString("录制完成 共 %1 帧").arg(fpsCount));
        btnStart->setText("开始");
...
void GIFWidget::resizeForm()
{
    ...
    int height = txtHeight->text().toInt();
    this->resize(width,
                 height + widgetTop->height() + widgetBottom->height());
}
