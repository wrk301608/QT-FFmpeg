...
#include "qfile.h"
QScopedPointer<CodeCount> CodeCount::self;

//定义单例模式，确保统计代码的时候只能有一个
CodeCount* CodeCount::getInstance()
{
    //还没有创建实例
    if(self.isNull())
    {
...
CodeCount::CodeCount(QWidget *parent) : QWidget(parent),ui(new Ui::CodeCount)
{
   ui->setupUi(this);
   this->initForm();
   on_btnClear_clicked();
...
bool CodeCount::checkFile(const QString &fileName)
{
...
void CodeCount::countCode(const QString &filePath)
{
    QDir dir(filePath);
...
//重写代码统计关闭页面的事件
void CodeCount::closeEvent(QCloseEvent *event)
{
    event->ignore();
    this->hide();
}

void CodeCount::countCode(const QStringList &files)
{
    int lineCode;
    int lineBlank;
    int lineNotes;
...
void CodeCount::countCode(const QString &fileName, int &lineCode, int &lineBlank, int &lineNotes)
{
...
void CodeCount::on_btnOpenFile_clicked()
{
...
void CodeCount::on_btnOpenPath_clicked()
{
...
void CodeCount::on_btnClear_clicked()
{
